...
List melt_dataframe(const DataFrame& data,
  ...
  if (!Rf_isNull(measure_attributes)) {
    // Copy attributes from measure_attributes to the output value column
    Rf_copyMostAttrib(measure_attributes, output[n_id + 1]);
    // Preserve the class attribute from the source data if it exists
    SEXP src_class = Rf_getAttrib(data[measure_ind[0]], R_ClassSymbol);
    if (!Rf_isNull(src_class)) {
      Rf_setAttrib(output[n_id + 1], R_ClassSymbol, src_class);
    }
  }
